...
- Chromium has been built **by hand on Linux only** — not in CI, never on Windows. The Windows
  and low-memory instructions are written and reasoned, not executed: the 35–50 h estimate for a
  6-core / 8 GB laptop is extrapolated from the single 17-core reference build, so the first real
  run should replace it with a measurement. The build is
